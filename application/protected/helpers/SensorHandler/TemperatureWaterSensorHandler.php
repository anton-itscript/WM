...
class TemperatureWaterSensorHandler extends TemperatureSensorHandler
{
    public $features = array(
        array(
            ...
            'feature_code'          => 'temperature_water', // feature code to be stored at table `station_sensor_feature`
            'measurement_type_code' => 'temperature_water', // measurement code to get possible metrics for.
            ...
            'aws_graph_using'       => 'Temperature Water',  // if takes part in “AWS Graph” page
            'aws_panel_show'        => 1,
        )
    );
    ...
    public function getInfoForAwsPanel($sensor_pairs, $sensorList, $sensorData, $for = 'panel')
    {
        ...
        if (isset($sensorList['temperature_water']) && is_array($sensorList['temperature_water']))
        {
            foreach ($sensor_ids as $sensor_id)
            {
                if (!isset($sensorList['temperature_water'][$sensor_id]))
                    continue;

                $sensorFeature = $sensorList['temperature_water'][$sensor_id];
        ...
        foreach ($return as $station_id => &$sensors)
        {
            ...
            {
                if (isset($sensorData['temperature_water'][$station_id][$sensor_id]) && (count($sensorData['temperature_water'][$station_id][$sensor_id]) > 0) &&
                    ($sensorData['temperature_water'][$station_id][$sensor_id][0]->listener_log_id == (isset($last_logs_per_station[$station_id][0]) ? $last_logs_per_station[$station_id][0] : -1)))
                {
                    $sensorValue = $sensorData['temperature_water'][$station_id][$sensor_id][0];

                    if ($sensorValue->is_m != 1)
                    {
                        $sensorValues['last'] = $this->formatValue($sensorValue->sensor_feature_value, 'temperature_water');
                        ...
                        if (count($sensorData['temperature_water'][$station_id][$sensor_id]) > 3)
                        {
                            $previousSensorValue = array();
                            for($i=0;$i<4;$i++)
                                $previousSensorValue[]=$sensorData['temperature_water'][$station_id][$sensor_id][$i]->sensor_feature_value;
                    ...
                    {
                        $maxmin = $this->getMaxMinInDay($sensor_feature_ids2[$sensor_id], strtotime($sensor_measuring_time[$sensor_id]), 1, 0, 'temperature_water');
                        $sensorValues['max24'] = $maxmin['max'];
                        $sensorValues['min24'] = $maxmin['min'];
                        $sensorValues['mami_title'] = $maxmin['mami_title'];
                        $maxmin = $this->getMaxMinInDay($sensor_feature_ids2[$sensor_id], strtotime($sensor_measuring_time[$sensor_id]), 2, 0, 'temperature_water');
    ...
    public function formatValue($value, $feature_name = 'temperature_water')
    {
        if ($feature_name === 'temperature_water')
        {
    ...
    public function _prepareDataPairs()
    {
        ...
        foreach($this->sensor_features_info as $feature) {
            if ($feature['feature_code'] == 'temperature_water') {
        ...
        $this->prepared_pairs[] = array(
            'feature_code' => 'temperature_water',
            'period'       => 1,
            'value'        => $value,
    ...
    public function getRandomValue($features) {
        ...
        $data = It::convertMetric($data, 'celsius', $features['temperature_water']);
    ...
    public function prepareXMLValue($xml_data, $db_features) {
        ...
        if ($xml_data['temperature_water'][0] == 'M') {
            $direction = 'M';
            $data = 'MMM';
        } else {
            $tmp = 10*It::convertMetric($xml_data['temperature_water'], 'celsius', $db_features['temperature_water']);
            $direction = $tmp > 0 ? '1' : '0';
